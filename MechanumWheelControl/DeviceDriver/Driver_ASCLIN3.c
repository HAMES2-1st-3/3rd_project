...
uint8 s_asclin3_tx_buf[ASC3_TX_BUFFER_SIZE + sizeof(Ifx_Fifo) + 8];
uint8 s_asclin3_rx_buf[ASC3_RX_BUFFER_SIZE + sizeof(Ifx_Fifo) + 8];
...
{
    IfxAsclin_Asc_blockingWrite(&s_asclin3, chr);
}

uint8 read_asclin3_uart(void)
{
    return IfxAsclin_Asc_blockingRead(&s_asclin3);
}
